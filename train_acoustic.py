...
from Models.transformer_acoustic import Transformer
...
def check_inf(predict_ts_ctc, text, mel_lengths_downsample, text_lengths, mel_name, blank=0):
    for ptc, t, mld, tl, ml in zip(predict_ts_ctc.transpose(0,1), text, mel_lengths_downsample, text_lengths, mel_name):
        loss_ctc = F.ctc_loss(ptc.unsqueeze(1), t.unsqueeze(0), mld.unsqueeze(0), tl, blank=0)
        if torch.isinf(loss_ctc):
            print(ml)
...
def get_ls_weight(T, L, mel_name, onehot):
    #  onehot = onehot * (1 - 0.1) + (1 - onehot) * 0.1 / (youtputs.size(2) - 1)
    for i, line in enumerate(mel_name):
        if 'mel_xvector_100_hop256_llm' in line:
            ls_weight = 0.25
        elif 'mel_xvector_100_hop256' in line:
            ls_weight = 0.2
        elif '/lmfb/':
            ls_weight = 0.1
        else:
            ...
            print('not expected')
            sys.exit(1)
        onehot[i*T:(i+1)*T] = onehot[i*T:(i+1)*T] * (1 - ls_weight) + (1 - onehot[i*T:(i+1)*T]) * ls_weight / (L - 1)
    return onehot

def dev_loop(model, optimizer, hp, rank, dataloader):
    model.train()
    optimizer.eval()
    with torch.no_grad():
        loss_att = 0.0
        for d in dataloader:
            text, mel_input, pos_text, pos_mel, text_lengths, mel_lengths, mel_name = d
            text = text.to(rank, non_blocking=True)
            mel_input = mel_input.to(rank, non_blocking=True)
            pos_text = pos_text.to(rank, non_blocking=True)
            pos_mel = pos_mel.to(rank, non_blocking=True)
            text_lengths = text_lengths.to(rank, non_blocking=True)
            ...
            if hp.decoder == 'LSTM' or hp.decoder.lower() == 'transformer':
                ys = text.contiguous().view(-1)
                loss_att_local = F.cross_entropy(youtputs.view(-1, youtputs.size(-1)), ys, ignore_index=0, reduction='mean').item()
            #print('dev loss_att =', loss_att_local))
            loss_att += loss_att_local

    print('dev loss_att =', loss_att / len(dataloader))
    model.eval()
    return
...
def train_loop(model, optimizer, step, epoch, args, hp, rank, dataloader):
    ...
    if hp.optimizer_type == 'Noam' or hp.optimizer_type == 'AdamW' or hp.optimizer_type.lower() == 'lion':
        if epoch >= hp.decay_epoch:
            ...
            if hp.optimizer_type.lower() == 'lion':
                lr /= 10.0
    ...
    for d in dataloader:
        ...
        with torch.cuda.amp.autocast(hp.amp): #and torch.autograd.set_detect_anomaly(True):
            if args.n_gpus > 1:
                dist.barrier()
            if hp.dev_mode:
                youtputs, ctc_outputs, attn_enc_enc, attn_dec_dec, attn_dec_enc, iter_preds = model(mel_input, text_input, src_mask, trg_mask, real_flag)
            else:
                youtputs, ctc_outputs, attn_enc_enc, attn_dec_dec, attn_dec_enc, iter_preds = model(mel_input, text_input, src_mask, trg_mask)
            ...
            if hp.decoder == 'LSTM' or hp.decoder.lower() == 'transformer':
                ...
                if label_smoothing:
                    ...
                    if hp.ls_weight:
                        onehot = get_ls_weight(T, L, mel_name, onehot)
                    else:
                        onehot = onehot * (1 - 0.1) + (1 - onehot) * 0.1 / (youtputs.size(2) - 1)
            ...
            elif hp.decoder.lower() == 'ctc':
                predict_ts_ctc = F.log_softmax(youtputs, dim=2).transpose(0, 1)
                mel_lengths_downsample = mel_lengths 
            ...
            elif hp.decoder.lower() == 'diffusion':
                loss_diffusion = youtputs[0]
                loss = loss_diffusion
            ...
            if hp.use_ctc:
                ...
                loss_ctc = F.ctc_loss(predict_ts_ctc, text, mel_lengths_downsample, text_lengths, blank=0, zero_infinity=True)
                print('loss_ctc = {}'.format(loss_ctc.item()))
                if not torch.isinf(loss_ctc):
                    loss = (hp.mtl_weight * loss + (1 - hp.mtl_weight) * loss_ctc)
                else:
                    check_inf(predict_ts_ctc, text, mel_lengths_downsample, text_lengths, mel_name, blank=0)
                    #print(mel_name)
        ...
        if not torch.isnan(loss):
            ...
            if step % hp.accum_grad == 0 and (hp.optimizer_type == 'Noam' or hp.optimizer_type == 'AdamW' or hp.optimizer_type.lower() == 'lion'):
                if epoch < hp.decay_epoch:
                    lr = get_learning_rate(step//hp.accum_grad+1, warmup_step=hp.warmup_step, warmup_factor=hp.warmup_factor, d_model=hp.d_model_e)
                    if hp.optimizer_type.lower() == 'lion':
                        lr /= 10.0
    ...
    if rank == 0 and ((epoch+1) % hp.save_per_epoch >= (hp.save_per_epoch - 10) or (epoch+1) % hp.save_per_epoch == 0) or (epoch+1) >= hp.average_start:
...
def get_dataloader(step, args, hp):
    if step // hp.accum_grad > hp.warmup_step:
        train_dataset = datasets_asr.get_dataset(hp.train_script, hp, spec_aug=hp.use_spec_aug, feat_norm=[hp.mean_file, hp.var_file], lengths_file=hp.lengths_file)
        lengths_file = hp.lengths_file
    else:
        train_dataset = datasets_asr.get_dataset(hp.train_script_first, hp, spec_aug=False, feat_norm=[hp.mean_file, hp.var_file], lengths_file=hp.lengths_file_first)
        lengths_file = hp.lengths_file_first
    #train_dataset = datasets_asr.get_dataset(hp.train_script, hp, spec_aug=hp.use_spec_aug, feat_norm=[hp.mean_file, hp.var_file])

    collate_fn_transformer = datasets_asr.collate_fn
    if hp.batch_size is not None:
        sampler = datasets_asr.NumBatchSampler(train_dataset, hp.batch_size)
    elif hp.max_seqlen is not None:
        sampler = datasets_asr.LengthsBatchSampler(train_dataset, hp.max_seqlen, lengths_file, shuffle=True, shuffle_one_time=False, shuffle_all=hp.dataset_shuffle_all, hugging_face=hp.hugging_face)

    train_sampler = datasets_asr.DistributedSamplerWrapper(sampler) if args.n_gpus > 1 else sampler
...
def train_epoch(model, optimizer, args, hp, step, start_epoch=0, rank=0):
    ...
    if step == 1 and start_epoch != 0:
        step = len(dataloader) * start_epoch + 1

    for epoch in range(start_epoch, hp.max_epoch):
        if hp.optimizer_type.lower() == 'schedule_free_adamw':
            model.train()
            optimizer.train()
        ...
        if hp.dev_script is not None:
            print('dev start')
            dev_dataset = datasets_asr.get_dataset(hp.dev_script, hp, spec_aug=False, feat_norm=[hp.mean_file, hp.var_file])
            dev_sampler = datasets_asr.NumBatchSampler(dev_dataset, 30)
            dev_sampler = datasets_asr.DistributedSamplerWrapper(dev_sampler) if args.n_gpus > 1 else dev_sampler
            dev_dataloader = DataLoader(dev_dataset, batch_sampler=dev_sampler, num_workers=8, collate_fn=datasets_asr.collate_fn)
            dev_loop(model, optimizer, hp, rank, dev_dataloader)
...
#
# 分散計算を行うためのユーティリティ関数
# 目的は、指定された関数 fn を指定した数のGPUで分散して実行すること
def run_distributed(fn, args, hp):
    # 現在の時刻を利用して、ポート番号を動的に生成
    port = '6' + str(int(time.time()))[-3:]
...
def run_training(rank, args, hp, port=None):
    ...
    if hp.optimizer_type == 'Noam':
        ...
        print(optimizer)
    elif hp.optimizer_type.lower() == 'lion':
        from lion_pytorch import Lion
        optimizer = Lion(model.parameters(), lr=max_lr, betas=(0.9, 0.98), weight_decay=1e-2)
        print(optimizer)
    elif hp.optimizer_type.lower() == 'schedule_free_adamw':
        import schedulefree
        optimizer = schedulefree.AdamWScheduleFree(model.parameters(), lr=max_lr) 
        print('schedule_free_adamw', optimizer)
    else:
        import dadaptation
        optimizer = dadaptation.DAdaptAdam(model.parameters(), lr=max_lr, betas=(0.9, 0.98), eps=1e-9, log_every=1)#torch.optim.Adam(model.parameters(), lr=max_lr)
        print('datapatation', optimizer)
    ...
    if hp.loaded_epoch is not None:
        ...
        if hp.is_flat_start:
            ...
            #train_dataset = datasets_asr.get_dataset(hp.train_script, hp, spec_aug=False, feat_norm=[hp.mean_file, hp.var_file])
            #collate_fn_transformer = datasets_asr.collate_fn
            #if hp.batch_size is not None:
            #    sampler = datasets_asr.NumBatchSampler(train_dataset, hp.batch_size)
            #elif hp.max_seqlen is not None:
            #    sampler = datasets_asr.LengthsBatchSampler(train_dataset, hp.max_seqlen, hp.lengths_file, shuffle=True, shuffle_one_time=False, shuffle_all=hp.dataset_shuffle_all)
            ...
            if hp.optimizer_type.lower() != 'schedule_free_adamw':
                step = int(loaded_dict['state'][0]['step'] * hp.accum_grad)
                lr = get_learning_rate(step//hp.accum_grad+1, warmup_step=hp.warmup_step, warmup_factor=hp.warmup_factor, d_model=hp.d_model_e)
                if hp.optimizer_type.lower() == 'lion':
                    lr /= 10.0
                for param_group in optimizer.param_groups:
