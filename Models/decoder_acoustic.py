...
class LSTMDecoder(nn.Module):
    ...
    def __init__(self, hp):
        ...
        # self.norm_gy = nn.LayerNorm(self.num_decoder_hidden_nodes, eps=1e-05, elementwise_affine=True)
        # self.norm_sy = nn.LayerNorm(self.num_decoder_hidden_nodes, eps=1e-05, elementwise_affine=True)
    ...
    def decode_v3(self, hbatch, src_mask, model_lm=None, lm_weight=0.2, model_lm_2=None, lm_weight_2=0.2, beam_width=10, version=3):
        ...
        for seq_step in range(max_decoder_seq_len):
            ...
            if seq_step == 0:
                ...
                k_scores, k_ix = scores.reshape(-1).topk(beam_width * 2)
                cand_idx = torch.div(k_ix, beam_width, rounding_mode='trunc') #k_ix // beam_width
                cand_ids = k_ix % beam_width
                ...
                while num_cand < beam_width:
                    if best_indices[cand_idx[i_cand], cand_ids[i_cand]] == eos_id:
                        if cand_seq[cand_idx[i_cand]][0] == 2:
                            # normalized
                            beam_results['score'][beam_step] = k_scores[i_cand] / (seq_step + 1)
                            beam_results['result'][beam_step] = cand_seq[cand_idx[i_cand]]
                            beam_results['result'][beam_step][seq_step] = best_indices[cand_idx[i_cand], cand_ids[i_cand]]
                            beam_results['length'][beam_step] = seq_step + 1
                            if not self.hp.multihead:
        ...
        results = []
        # acoustic = []
        for i in range(len(beam_results['score'])):
            length = beam_results['length'][i]
            results.append(beam_results['result'][i][:length].cpu().tolist())

        return results, beam_results['score']
    ...
    def decode_v2(self, hbatch, src_mask, model_lm=None, lm_weight=0.2, model_lm_2=None, lm_weight_2=0.2, beam_width=10):
        ...
        for enc in hbatch[0]:
            ...
            while True:
                ...
                # print(new_hyp['result'])
                # import pdb; pdb.set_trace()
                A_hyps.remove(new_hyp)

                # lm_hidden = new_hyp['lm_hidden']

                ys = torch.tensor([new_hyp['result'][-1]]).long().unsqueeze(0).to(device)
                ...
                ## 多分ここが間違ってる: greedy searchでは、blank時に更新していない
                # import pdb; pdb.set_trace()
                ...
                # calculate score without blank
                top_k = ytu[1:].topk(k_range, dim=-1)

                # to add blank score
                beam_hyp = {'score': new_hyp['score'] + float(ytu[0:1]),
                            'result': new_hyp['result'][:],
                            'cache': new_hyp['cache']}
                B_hyps.append(beam_hyp)
                
                if model_lm is not None:
                    lm_input = torch.tensor(new_hyp['result']).long().unsqueeze(0).to(device)
                    lm_score = model_lm(lm_input)

                for logp, k in zip(*top_k):
                    score = new_hyp['score'] + float(logp)

                    if model_lm is not None:
                        # import pdb; pdb.set_trace()
                        score += lm_weight * float(F.log_softmax(lm_score[:, -1, :], dim=1)[0, k+1])

                    # if (k+1) in new_hyp['result']:
                    #     pass
                    # else:
                        # print(new_hyp['result'][:] + [int(k)+1])
                    A_hyps.append({'score': score,
                                    'result': new_hyp['result'][:] + [int(k)+1],
                                    'cache': h})
                    
                # import pdb;pdb.set_trace()
                hyps_max = float(max(A_hyps, key=lambda x: x['score'])['score'])
                # import pdb; pdb.set_trace()
                kept_most_prob = sorted([hyp for hyp in B_hyps if hyp['score'] > hyps_max], key=lambda x: x['score'])
                # print(len(kept_most_prob))
                # import pdb; pdb.set_trace()
                if len(kept_most_prob) >= k_range:
                    B_hyps = kept_most_prob
                    break

        nbest_hyps = sorted(B_hyps, key=lambda x: x['score'] / len(x['result']), reverse=True)[:nbest]

        return nbest_hyps[0]['result'][1:]


class DiffTransformerDecoder(nn.Module):
    """
    Transformer decoder.
    Args (from hparams.py):
        vocab_size (int): vocabulary size
...
class DiffDecoder(nn.Module):
    """
    Decoder for the diffusion model.
    Diffuion model for discrete time series data.
    """
    def __init__(self, hp):
        super(DiffDecoder, self).__init__()
        self.decoder = DiffTransformerDecoder(hp)
        self.out = nn.Linear(hp.d_model_d, hp.vocab_size)
        self.hp = hp

    def forward(self, targets, hbatch, src_mask, trg_mask):
        loss = self.loss_fn(targets, hbatch, src_mask, trg_mask)
        return loss

    def recognize(self, hbatch, src_mask, ctc_out, model_lm=None, lm_weight=0.2, model_lm_2=None, lm_weight_2=0.2, beam_width=10, n_timesteps=20):
    # def decode_v2(self, hbatch, src_mask, model_lm=None, lm_weight=0.2, model_lm_2=None, lm_weight_2=0.2, beam_width=10):
        # Get X_0 hat
        batch_size = hbatch.shape[0]
        ctc_results = []
        prev_id = self.hp.vocab_size + 1
        text_lengths = torch.zeros((batch_size, 1), device=hbatch.device, requires_grad=False)
        for b in range(batch_size):
            results_batch = []
            for x in ctc_out[b].argmax(dim=1):
                if int(x) != prev_id and int(x) != 0:
                    results_batch.append(int(x))
                    text_lengths[b] += 1
                prev_id = int(x)
        ctc_results = results_batch
        
        # text_lengths
        #text_mask = torch.arange(text_lengths.max()).to(x.device).expand(len(text_lengths), text_lengths.max())
        #text_mask = text_mask < text_lengths[:, None]
        # TODO: multple batch size
        import sentencepiece as spm
        sp = spm.SentencePieceProcessor()
        sp.Load(self.hp.spm_model)

        masked_targets = torch.zeros((batch_size, text_lengths[0][0].int()), device=hbatch.device, dtype=torch.long)
        for t in range(n_timesteps):
            out, _, _ = self.decoder(masked_targets, hbatch, src_mask, trg_mask=None)
            out = self.out(out)
            confidences, results = out.softmax(dim=-1).max(dim=-1)
            masked_targets = torch.where(torch.bernoulli(confidences)==1, results, torch.zeros_like(results))
        return results[0].tolist()

    def forward_step(self, targets, hbatch, t, src_mask, trg_mask, text_lengths=None):
        """
        Forward step.
        Args:
            x: input tensor of linguistic features (B, seq_len)
            t: time step (B)
        Returns:
            out: output tensor
        """
        ...
        #text_mask = torch.arange(text_lengths.max()).to(x.device).expand(len(text_lengths), text_lengths.max())
        #text_mask = text_mask < text_lengths[:, None]
        mask = bernoulli_sample(t, trg_mask)
        # 
        masked_targets = (targets * mask).long() # x (batch_size, seq_len)
        ## TODO
        # forward(self, targets, hbatch, src_mask, trg_mask)
        #def forward(self, trg, e_outputs, src_mask, trg_mask):
        #import pdb; pdb.set_trace()
        trg_mask = trg_mask[:,-1,:].unsqueeze(1)
        out, _, _ = self.decoder(masked_targets, hbatch, src_mask, trg_mask)
        out = self.out(out)
        return out, mask, masked_targets

    def loss_fn(self, targets, hbatch, src_mask, trg_mask):
        """
        Loss function.
        Args:
            x: input tensor
        ...
        #loss = nn.MSELoss()
        # time step
        t = torch.rand((hbatch.shape[0], 1)).to(hbatch.device)
        out, mask, masked_targets = self.forward_step(targets, hbatch, t, src_mask, trg_mask)

        #prediction, mask = self.forward_step(x, t)
        unmasked_targets = (targets * (1-mask)).long()
        ## TODO: How do we set the label?
        loss = nn.functional.cross_entropy(out.transpose(1, 2), unmasked_targets, label_smoothing=0.1, ignore_index=0)
        return (loss, out), None, None


def bernoulli_sample(t, text_mask):
    """
    Bernoulli sample.
    Args:
        t: time step (B, 1)
        text_mask: text mask (B, T, T)
    """
    t = t.expand(-1, text_mask.shape[1])
    #t = t.expand(text_mask.shape[1])
    x = torch.sin(torch.pi * t / 2)
    #x =
