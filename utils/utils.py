...
def create_masks(src_pos, trg_pos, src_pad=0, trg_pad=0):
    ...
    if trg_pos is not None:
        ...
        #if trg_pos.is_cuda:
        np_mask = np_mask.to(trg_pos.device)
...
def fill_variables(hp, verbose=True):
    default_var = {'pe_alpha': False, 'stop_lr_change': 100000000, 'feed_forward': 'linear', 'optimizer': 'adam',
                   'mel_dim':80, 'is_flat_start':False,'dataset_shuffle_all':False, 'optimizer_type': 'Noam',
                   'init_lr':1e-3, 'save_per_epoch': 50, 'save_attention_per_step': 2000, 'num_F':2,
                   'accum_grad':1, 'N_e':12, 'N_d':6, 'heads':4, 'd_model_e':256, 'd_model_d':256, 'encoder': None,
                   'amp': False, 'comment':'', 'granularity':1, 'subsampling_rate': 4, 'frame_stacking':1,
                   'decoder_rel_pos':False, 'random_mask':False, 'decoder': 'Transformer', 'cnn_avepool':False,
                   'decay_epoch': 100000, 'mean_utt':False, 'multihead':False, 'l1_flag':False, 'load_name_lm': None,
                   'use_ctc':False, 'lm_weight': None, 'cnn_swish':False, 'cnn_ln': False, 'beam_width':10, 'rms_norm': False,
                   'ls_weight': False, 'silence_files':None, 'train_script_first': hp.train_script, 'lengths_file_first': hp.lengths_file,
                   'rotary_pos_enc': False}
    ...
    dev_var = {'swish_lstm':False, 'norm_lstm':False, 'load_name_lm_2':None, 'weight_dropout':None, 'dev_mode': None, 'iter_loss':[], 'decoder_swish':False, 'use_lm_loss':False, 'eps_lm_loss': 0.0, 'n_model_d':2, 
               'use_aux_transducer_loss':False, 'use_symm_kl_div_loss':False, 'separate_tts_specaugment': True, 'batchnorm_momentum':0.1, 'dev_bn': False, 'hugging_face': False}
