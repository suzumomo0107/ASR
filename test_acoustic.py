...
def recognize(hp, model, script_file, model_lm, lm_weight, model_lm_2=None, lm_weight_2=None, calc_wer=False):
    ...
    for i in range(num_mb):
        ...
        if src_seq.shape[1] >= args.segment:
            ...
            youtput_in_Variable, acoustic = model.decode(src_seq, src_seq_dummy, hp.beam_width, model_lm, INIT_TOK, EOS_TOK, lm_weight, model_lm_2, lm_weight_2)
            if len(youtput_in_Variable) != 0:
                result_print += f"{sp.DecodeIds(youtput_in_Variable)}"
                # result_print += f"{sp.DecodeIds(youtput_in_Variable[1])}" ##here
        ...
        acoustic_list = [str(i) for i in acoustic]   

        if not calc_wer:
            print(result_print)
            print(f"acoustic_score {acoustic_list}")
            sys.stdout.flush()
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--wo_lm', action='store_true')
    ...
    if hp_file is None:
        ...
        #hp_file = os.path.join('/work2/suzumomo/ASR_sensei/checkpoints/LibriSpeech/checkpoints.conformer-LSTM.LibriSpeech_A6000_Ada/hparams.py')
    ...
    if args.load_name_lm is not None and args.wo_lm is False:
    ...
    if hp.load_name_lm is not None and args.wo_lm is False:
